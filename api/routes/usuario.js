...
const validaUsuario = [
    ...
    .isAlpha('pt-BR', {ignore: ' '}).withMessage('Informe apenas o texto no nome')
    .isLength({ min: 3 }).withMessage('O nome do usuário deve ter ao menos 3 caracteres')
    .isLength({ max: 100 }).withMessage('O nome do usuário deve ter no máximo 100 caracteres'), check('email')
    ...
    .custom((value, {req}) => {
        return db.collection(nomeCollection).find({email: {$eq: value}}).toAwrray()
        .then((email) => {
            //verifica se não tem o id, para garantir que é inclusão
            if (email.legth && !req.params.id){
    ...
    check('senha')
        ...
        .isStrongPassword({minLength: 6,
                           ...
                           minSymbols: 1, minNumbers: 1}).withMessage('A senha informada não é segura. Informe no mínimo 1 caractere maiúsculo, 1 caractere minúsculo, 1 número e 1 caractere especial'),
    check('ativo')
        .default(true)
        .isBoolean().withMessage('O valor deve ser um booleano. True ou False'),
    chack('tipo')
        .default('Clientes')
        .isIn(['Admin', 'Cliente']).withMssage('O tipo do usuário deve ser Admin ou Cliente'),
    check('avatar')
        ...
        router.post('/', validaUsuario, async(req, res) => {
            ...
            if (!schemaErrors.isEmpty()){
                ...
                req.body.avatar = `http://ui-avatars.com/api/?name=${req.body.nome.replace(/ /g,'+')}& background=F00&color=00F`
